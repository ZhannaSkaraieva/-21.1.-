...
console.log('#4. JavaScript homework example file')
...
function originalFunction(num) {
  return num * 4;     //я не поняла почему получается ответ только с 4 кой
}

function modifyFunction (originalFunction , multiplier) {  //Higher-Order Functions (HoF)
    return function (num) {
        return originalFunction(num) * multiplier;
    }
}
//const modifiedFunc = function (num)
//return originalFunction(num) * multiplier;
...
//const protocolSetter = function (domainName) {
//       return function (tld) {
//        return `${protocol}://${domainName}.${tld}`;  
//     }
// };
...
//const domainNameSetter = function (tld) {
//        return `${protocol}://${domainName}.${tld}`;  
//     }
...
const result1 = addSecond(3) // Повинно повернути 6
